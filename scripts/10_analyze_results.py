"""Stage 10: Summarize docking results and compare with known inhibitor dataset statistics.

Adapted from original notebook cell 043; paths have relative fallbacks for the GitHub repo layout.
"""
import pandas as pd
...
denovo_df = pd.read_csv("results/docking/vina_scores_sorted.csv") if __import__("os").path.exists("results/docking/vina_scores_sorted.csv") else pd.read_csv("vina_scores_sorted.csv") # This is your 84.5-min run

# 2. Load the KNOWN DRUGS (your fine-tuning set)
baseline_df = pd.read_csv("data/processed/chembl_hiv_protease_clean.csv") if __import__("os").path.exists("data/processed/chembl_hiv_protease_clean.csv") else pd.read_csv("chembl_hiv_protease_clean.csv")
